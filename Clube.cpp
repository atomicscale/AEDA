...
bool Clube::removeFornecedor(const string nome) {
	priority_queue<Fornecedor> temp;
	while (!fornecedores.empty()) {
		if (fornecedores.top().getNome() != nome) {
			temp.push(fornecedores.top());
			fornecedores.pop();
		} else
			fornecedores.pop();
	}
	setFornecedores(temp);
	return true;
}
...
void Clube::criarJogador() {
	...
	try {
		...
		Jogador* jogador = new Jogador(name, idade, sexo, nif, salario,
				duracao_contrato);
...
void Clube::criarModalidades() {
	...
	float quota(0);
	int nr_jogadores(0);
	input_field("Deseja criar uma modalidade ou uma sub-modalidade? (M/S)", 0,
			resposta);
	try {
		if (resposta == "m" || resposta == "M") {
			...
			for (string::iterator it = modalidade.begin();
					it != modalidade.end(); ++it) {
				if (((*it) < 65 || (*it) > 90) && ((*it) < 97 || (*it) > 122)) {
					throw InputInvalido();
				}
			}
			...
			input_field("Introduza o nome da Modalidade  a procurar", 48,
					modalidade);

			bool encontrado = false;
			for (vector<Modalidade*>::iterator it = modalidades.begin();
					it != modalidades.end(); ++it) {
			...
			input_field("Introduza o nome da Sub-Modalidade", 48,
					submodalidade);

			input_field("Introduza o nome do Treinador da Sub-Modalidade", 48,
					treinador);

			for (string::iterator it = treinador.begin(); it != treinador.end();
					++it) {
				if (((*it) < 65 || (*it) > 90) && ((*it) < 97 || (*it) > 122)) {
					throw InputInvalido();
				}
			...
			input_field("Introduza o numero de Jogadores da Sub-Modalidade", 48,
					nr_jogadores);
			...
			for (auto m : modalidades) {
				if (m->getNome() == modalidade) {
					SubModalidade* sm = new SubModalidade(submodalidade,
							treinador, nr_jogadores);
					m->adicionaSub(sm);
					break;
				}
			}
		...
		cerr << "Modalidade inexistente. Processo de criacao cancelado."
				<< endl;
	} catch (SubModalidadeExistente &) {
		cerr << "Submodalidade ja existe. Processo de criacao cancelado."
				<< endl;
	} catch (InputInvalido &) {
		cerr << "Input invalido. Processo de criacao cancelado." << endl;
	}
...
void Clube::criarSocios() {
	...
	try {
		...
		cerr << "Erro! S�cio : " << e.getName()
				<< " inexistente.Processo de criacao cancelado. " << endl;
	}
...
void Clube::atribuirModalidadeaSocio() {
	...
	try {
		std::cout << "Introduza o nome do Socio a atribuir uma modalidade"
				<< endl;
		cin.ignore();
		...
		if (socioIndex == -1)
			throw SocioInexistente(tempSocio);
		for (string::iterator it = tempSocio.begin(); it != tempSocio.end();
				++it) {
			if (((*it) < 65 || (*it) > 90) && ((*it) < 97 || (*it) > 122)) {
				throw InputInvalido();
			}
		...
		// substituir pelo vetor do S�cio
		socios[socioIndex]->setModalidades(tempMod);

	} catch (SocioInexistente &e) {
		cerr << "Erro! O Socio  " << "(" << e.getName() << ")"
				<< " Nao Existe! Processo de criacao cancelado. " << endl;
	} catch (InputInvalido &) {
		cerr << "Input invalido. Processo de criacao cancelado." << endl;
...
void Clube::atribuirModalidadeaJogador() {
	...
	cin.exceptions(istream::failbit | istream::badbit);
	try {
		std::cout << "Introduza o nome do Jogador a atribuir uma modalidade"
				<< endl;
		cin.ignore();
		...
		if (jogadorIndex == -1)
			throw JogadorInexistente(tempJogador);
...
template<typename T>
void createTable(string tableName, vector<pair<string, unsigned int> > colunas,
		vector<T*> content) {
	...
	std::cout << string(1, (char) 201) << string(20, (char) 205)
			<< string(1, (char) 203) << string(6, (char) 205)
			<< string(1, (char) 203) << string(10, (char) 205)
			<< string(1, (char) 203) << string(10, (char) 205)
			<< string(1, (char) 203) << string(8, (char) 205)
			<< string(1, (char) 203) << string(9, (char) 205)
			<< string(1, (char) 187) << std::endl;

	for (unsigned int i(0); i < colunas.size(); i++) {
		std::cout << vericalSeparator << setw(colunas[i].second) << std::left
				<< colunas[i].first;
	}

	std::cout << vericalSeparator << std::endl;

	std::cout << string(1, (char) 204) << string(20, (char) 205)
			<< string(1, (char) 206) << string(6, (char) 205)
			<< string(1, (char) 206) << string(10, (char) 205)
			<< string(1, (char) 206) << string(10, (char) 205)
			<< string(1, (char) 206) << string(8, (char) 205)
			<< string(1, (char) 206) << string(9, (char) 205)
			<< string(1, (char) 185) << std::endl;

	for (unsigned int i = 0; i < content.size(); i++) {
		content[i]->imprime();
	}

	std::cout << string(1, (char) 200) << string(20, (char) 205)
			<< string(1, (char) 202) << string(6, (char) 205)
			<< string(1, (char) 202) << string(10, (char) 205)
			<< string(1, (char) 202) << string(10, (char) 205)
			<< string(1, (char) 202) << string(8, (char) 205)
			<< string(1, (char) 202) << string(9, (char) 205)
			<< string(1, (char) 188) << std::endl << std::endl;
}

template<typename T>
void createTableM(string tableName, vector<pair<string, unsigned int> > colunas,
		vector<T*> content) {
	string vericalSeparator(1, (char) 186);
	...
	for (unsigned int i(0); i < colunas.size() - 1; i++)
		std::cout << string(colunas[i].second, (char) 205)
				<< string(1, (char) 203);

	std::cout << string(colunas.back().second, (char) 205)
			<< string(1, (char) 187) << std::endl;
...
void Clube::listJogador() {
	...
	if (jogadores.empty()) {
		message("Erro, Nenhum Jogador Encontrado. Adicione um novo Jogador!",
				0);
	} else {
		for (unsigned int j = jogadores.size() - 1; j > 0; j--) {
			for (unsigned int i = 0; i < j; i++) {
				if (jogadores[i + 1]->getSalario()
						< jogadores[i]->getSalario()) {
		...
		pair<string, unsigned int> cols[] = { make_pair("Nome", 20), make_pair(
				"Idade", 6), make_pair("Nif", 10), make_pair("Sexo", 10),
				make_pair("Salario", 8), make_pair("Contrato", 9) };
...
void Clube::listModalidades() {
	if (modalidades.empty()) {
		message(
				"Erro, Nenhuma Modalidade Encontrada. Crie Uma Nova Modalidade!",
				0);
	} else {
		for (unsigned int j = modalidades.size() - 1; j > 0; j--) {
			for (unsigned int i = 0; i < j; i++) {
				if (modalidades[i + 1]->getQuota()
						< modalidades[i]->getQuota()) {
...
void Clube::listSocios() {
	if (socios.empty()) {
		...
		pair<string, unsigned int> cols[] = { make_pair("Nome", 20), make_pair(
				"Idade", 6), make_pair("Sexo", 10), make_pair("Nif", 10),
				make_pair("Prazo", 8), make_pair("N. Modal.", 9) };
		vector<pair<string, unsigned int> > colunas(cols,
				cols + sizeof(cols) / sizeof(pair<string, unsigned int> ));
...
void Clube::saveInfo() {
	...
	gravarFornecedores(fornecedores_file);
...
void Clube::loadInfo() {
	...
	lerFornecedores(fornecedores_file);
...
ifstream & Clube::lerJogadores(ifstream &i) {
	...
	while (getline(i, temp)) {
		...
		jogadores.push_back(
				new Jogador(vec[0], atoi(vec[1].c_str()), vec[3],
						atoi(vec[2].c_str()), atoi(vec[4].c_str()),
						atoi(vec[5].c_str())));
	}
...
ofstream & Clube::gravarJogadores(ofstream &o) {
	...
	o << setw(20) << std::left << "Nome" << setw(6) << std::left << "Idade"
			...
			<< setw(8) << std::left << "Salario" << setw(9) << std::left
			<< "Contrato" << std::endl;
...
ofstream & Clube::gravarModalidades(ofstream &o) {
	...
	o << setw(20) << std::left << "Nome" << setw(6) << std::left << "Quota"
			<< std::endl;
...
ifstream & Clube::lerModalidades(ifstream &i) {
	...
	while (getline(i, temp)) {
		...
		modalidades.push_back(
				new Modalidade(vec[0], float(atof(vec[1].c_str()))));
	}
...
ifstream & Clube::lerSocios(ifstream &i) {
	...
	while (getline(i, temp)) {
		...
		socios.push_back(
				new Socio(vec[0], atoi(vec[1].c_str()), vec[3],
						atoi(vec[2].c_str()),
						(vec[4].c_str() == "true" ? 1 : 0)));
	}
...
ofstream & Clube::gravarSocios(ofstream &o) {
	o << setw(20) << std::left << "Nome" << setw(6) << std::left << "Idade"
			<< setw(10) << std::left << "Sexo" << setw(10) << std::left << "Nif"
			<< setw(8) << std::left << "Prazo" << setw(9) << std::left
			<< "N. Modal." << std::endl;
...
void Clube::clubeInterface() {
	...
	do {
		...
		std::cout << std::setw(60) << std::right
				<< "--- Gestao de um Clube Desportivo ---" << std::endl
				<< std::endl;
		time_t now = time(0);
		tm *ltm = localtime(&now);
		// print various components of tm structure.
		std::cout << "Data: " << ltm->tm_mday << " de "
				<< getMonth(1 + ltm->tm_mon) << " de " << 1900 + ltm->tm_year
				<< endl;
		std::cout << "Hora: " << ltm->tm_hour << ":";
		std::cout << 1 + ltm->tm_min << ":";
		std::cout << 1 + ltm->tm_sec << std::endl << std::endl;
		std::cout << std::setw(12) << std::left << "Nome: " << nome
				<< std::endl;
		std::cout << std::setw(12) << std::left << "Presidente: " << presidente
				<< std::endl;
		std::cout << std::setw(12) << std::left << "Saldo: " << saldo << " $"
				<< std::endl << std::endl;
		...
		std::cout << tab() << "5. Atribuir a Jogador uma modalidade"
				<< std::endl;
		...
		std::cout << tab() << "10. Atribuir a Socio uma modalidade"
				<< std::endl;
...
void Clube::criarFornecedor() {
	string nome, sexo, servicos;
	int idade, nif, localizacao;

	system("cls");
	try {
		header("Criar Fornecedor", 0);
		message("Introduza o nome do Fornecedor", 0);
		cin.ignore();
		getline(std::cin, nome);
		for (string::iterator it = nome.begin(); it != nome.end(); ++it) {
			if (((*it) < 65 || (*it) > 90) && ((*it) < 97 || (*it) > 122)) {
		...
		input_field("Idade do Fornecedor", 0, idade);

		if (idade == 0 || idade < 0)
			throw InputInvalido();

		input_field("Nif do Fornecedor", 0, nif);
		...
		input_field("Servicos prestados", 0, servicos);
		if (servicos.length() == 0) {
			// throw ServicoInvalido();
		}

		input_field("Localizacao", 0, localizacao);

		Fornecedor f = Fornecedor(nome, idade, sexo, nif);
		f.setServicos(servicos);
...
void Clube::listFornecedores() {
	if (fornecedores.empty()) {
		message(
				"Erro, Nenhum Fornecedor Encontrado. Adicione um novo Fornecedor!",
				0);
	} else {
		priority_queue<Fornecedor> temp = fornecedores;
		vector<Fornecedor *> vec;
		while (!temp.empty()) {
			string nome, sexo;
			int idade = 0, nif = 0;
			nome = temp.top().getNome();
			sexo = temp.top().getSexo();
			idade = temp.top().getIdade();
			nif = temp.top().getNif();
			Fornecedor* f = new Fornecedor(nome, idade, sexo, nif);
			f->setLocalizacao(temp.top().getLocalizacao());
			f->setServicos(temp.top().getServicos());
			vec.push_back(f);
			temp.pop();
		}
		pair<string, unsigned int> cols[] =
				{ make_pair("Nome", 20), make_pair("Idade", 6), make_pair("Nif",
						10), make_pair("Sexo", 10), make_pair(
						"Servicos prestados", 8), make_pair("Localizacao", 9) };
		vector<pair<string, unsigned int> > colunas(cols,
				cols + sizeof(cols) / sizeof(pair<string, unsigned int> ));

		createTable("Lista de Fornecedores", colunas, vec);
	}

}

void Clube::removeFornecedorInterface() {
	string nome;
	if (fornecedores.size() == 0) {
		message("Nenhum Fornecedor Encontrado", 0);
	} else {
		try {

			input_field("Introduza o nome do Fornecedor a remover", 0, nome);
			for (string::iterator it = nome.begin(); it != nome.end(); ++it) {
				// if (((*it) < 65 || (*it) > 90) && ((*it) < 97 || (*it) > 122)){
				if (!isalpha(*it)) {
					throw InputInvalido();
			...
			if (removeFornecedor(nome))
				message("Fornecedor Removido Com Sucesso!", 0);
			else
				message("Fornecedor nao encontrado, tente novamente!", 0);
...
ifstream & Clube::lerFornecedores(ifstream & i) {
	std::string temp;
	std::vector<std::string> vec;
	istringstream iss;
	...
	while (getline(i, temp)) {
		...
		// fornecedores.push( Fornecedor (vec[0], float(atof(vec[1].c_str()))));  Precisas de ver isto
	}

	return i;
}

ofstream & Clube::gravarFornecedores(ofstream & o) {
	priority_queue<Fornecedor> temp = fornecedores;
